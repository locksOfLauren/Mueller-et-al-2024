function [cellResponses,x1,x2] = LM_McKenzie_medianFr_beh_dp(sd,varargin)

%no zscore, median FR

%function [cellResponses_free,cellResponses_forced,cellResponses_forced_excluded,x1,x2] = LM_GenerateTrialEnsembleVectors_trialSpecific(sd,varargin)

%create cellResponses for both forced-choice trials and closest free-choice trials 
dt = 0.1; %firing rate bin size (s), for plotting dp across trial
minTrials = 1; %15
...
% try for dp beh corr, completed trials 
         keep = sd{iS}.beh.trialOutcome==3&...%original,
             (sd{iS}.beh.blockNumber>1);%

            
             %changed 032824   
             % keep = sd{iS}.beh.trialOutcome==3&...%try, beh is sig.
             %  ~(sd{iS}.beh.error==1)&...
             % (sd{iS}.beh.blockTrialNumber>1&sd{iS}.beh.blockTrialNumber<101);
